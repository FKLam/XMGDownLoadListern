...
- (void)setState:(XMGTodayFireVoiceCellState)state {
    ...
    NSDictionary *bundleInfoDic = currentBundle.infoDictionary;
    NSString *bundleName = bundleInfoDic[@"CFBundleName"];
    switch (state) {
        case XMGTodayFireVoiceCellStateWaitDownLoad:{
            NSLog(@"等待下载");
            [self removeRotationAnimation];
            NSString *imagePath = [currentBundle pathForResource:@"cell_download.png" ofType:nil inDirectory:[NSString stringWithFormat:@"%@.bundle", bundleName]];
            [self.downLoadBtn setImage:[UIImage imageWithContentsOfFile:imagePath] forState:UIControlStateNormal];
        }
            break;
        case XMGTodayFireVoiceCellStateDownLoading:
        {
            NSLog(@"正在下载");
            NSString *imagePath = [currentBundle pathForResource:@"cell_download_loading@2x.png" ofType:nil inDirectory:[NSString stringWithFormat:@"%@.bundle", bundleName]];
             [self.downLoadBtn setImage:[UIImage imageWithContentsOfFile:imagePath] forState:UIControlStateNormal];
        ...
        case XMGTodayFireVoiceCellStateDownLoaded: {
            NSLog(@"下载完毕");
            [self removeRotationAnimation];
            NSString *imagePath = [currentBundle pathForResource:@"cell_downloaded@2x.png" ofType:nil inDirectory:[NSString stringWithFormat:@"%@.bundle", bundleName]];
            [self.downLoadBtn setImage:[UIImage imageWithContentsOfFile:imagePath] forState:UIControlStateNormal];
        }
            break;
        default:
